...
| 0.5B GQA GRPO | 544.67M | ✅ 已完成 | [leixinlin/MiniMind2-0.5B-GRPO](https://huggingface.co/leixinlin/MiniMind2-0.5B-GRPO) |
| 0.5B MLA 消融 | — | ❌ 未执行 | — |
...
| 200 | 0.0324 | -1.5512 | 394.44 |
| 1210 | 0.0697 | -0.6252 | 11.25 |
| 1219 | 0.0001 | -0.2169 | 178.19 |

#### 评测结果

**C-Eval（52 科 val 集）：**

| 类别 | SFT | DPO | GRPO | 变化(SFT→GRPO) |
|---|---|---|---|---|
| STEM | 26.59% | 26.30% | 26.30% | -0.29% |
| 社会科学 | 23.41% | 23.41% | 23.75% | +0.34% |
| 人文 | 32.86% | 32.86% | 32.86% | 0% |
| 其他 | 22.79% | 22.99% | 22.59% | -0.20% |
| **总体** | **26.00%** | **26.00%** | **25.93%** | **-0.07%** |

**生成质量对比（15 prompt）：**

| 能力维度 | SFT | DPO | GRPO | 变化(SFT→GRPO) |
|---|---|---|---|---|
| 事实问答 | 76.2 | 76.2 | 76.2 | 0 |
| 科学解释 | 89.3 | 89.3 | 89.3 | 0 |
| 逻辑推理 | 71.0 | 71.0 | 71.0 | 0 |
| 代码生成 | 92.0 | 92.0 | 92.0 | 0 |
| 创意写作 | 88.0 | 88.0 | **93.0** | **+5.0** |
| 自我认知 | 84.0 | 84.0 | 84.0 | 0 |
| **加权平均** | **82.87** | **82.87** | **83.53** | **+0.66** |

**推理效率：**

| 指标 | SFT | DPO | GRPO |
|---|---|---|---|
| 推理速度 | ~39-43 tok/s | ~38-43 tok/s | ~38-41 tok/s |
| 推理峰值显存 | 2173-2206 MB | 2173-2206 MB | 2173-2206 MB |

**GRPO 分析**：
- Reward 从 -0.98（step 10）逐步改善，最终为 -0.22（step 1219），说明模型在奖励信号引导下持续优化
- C-Eval 与 SFT 基本一致（25.93% vs 26.00%），GRPO 未损害知识能力
- 生成评分略有提升（82.87 → 83.53），创意写作改善最明显（88.0 → 93.0）
- Actor Loss 波动较大（0.0001 ~ 0.28），属 GRPO 正常现象（与采样质量相关）
...
- [x] 0.5B GQA GRPO 后训练 + 评测 + HuggingFace 上传
